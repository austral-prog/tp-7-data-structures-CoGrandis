...
def find_max_value(diccionario):
    ...
    if diccionario == {} :
        return ''
    max_value = list(diccionario.values())
...
def reverse_dict(diccionario):
    ...
    """
    reverse = {}
    for index, item in diccionario.items():
        if item not in reverse:
            reverse[item] = index
        else:
            reverse[item] = reverse[item] + index
...
def find_biggest_expense(gastos):
    ...
    mayor_promedio = 0
    categoria = ""
    for index, gasto in gastos.items():
        promedio = 0
        for item in gasto:
            promedio += item
...
def sum_expenses_by_type(gastos):
    ...
    for index, gasto in gastos.items():
        ...
        for item in gasto:
            if item[0] not in diccionario_gastos:
                diccionario_gastos[item[0]] = item[1]
            else:
                diccionario_gastos[item[0]] += item[1]

    return diccionario_gastos
